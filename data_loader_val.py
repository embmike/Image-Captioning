...
def get_loader(transform,
    ...
    if mode == 'train':
        if vocab_from_file==True: assert os.path.exists(vocab_file), \
            "vocab_file does not exist.  Change vocab_from_file to False to create vocab_file."
        img_folder = os.path.join(cocoapi_loc, 'cocoapi/images/train2014/')
        annotations_file = os.path.join(cocoapi_loc, 'cocoapi/annotations/captions_train2014.json')
        
    elif mode == 'val':
        assert batch_size==1, "Please change batch_size to 1 if validate your model."
        assert os.path.exists(vocab_file), "Must first generate vocab.pkl from validation data."
